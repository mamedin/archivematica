...
def bind_pid(**kwargs):
    ...
    """
    _validate(kwargs)
    resolve_url, qualified_resolve_urls, purl_map = _render_url_templates(
        kwargs)
    request_body = _render_request_body(kwargs, resolve_url,
                                        qualified_resolve_urls)
...
def _add_parser_args(parser):
    parser.add_argument('desired_pid', type=str, action='store',
                        help='The PID that you want to create or whose resolve'
                        ' URL you want to modify.', default=None)
    parser.add_argument('entity_type', type=str, action='store',
                        help='The type of entity---file or unit---that you want'
                        ' to create a PID for', choices=tuple(ENTITY_TYPES.keys()),
                        default=None)
    parser.add_argument('-a', '--naming-authority', type=str, action='store',
                        dest='naming_authority', help='A handle naming'
                        ' authority that you can create PIDs under, e.g.,'
                        ' 12345.')
    parser.add_argument('-e', '--pid-web-service-endpoint', type=str,
                        action='store', dest='pid_web_service_endpoint',
                        help='The URL to make requests to in order to create a'
                        ' PID and/or resolve a PID to a specific URL.')
    parser.add_argument('-k', '--pid-web-service-key', type=str,
                        action='store', dest='pid_web_service_key',
                        help='The web service key needed in order to make'
                        ' PID-creation requests to [PID_WEB_SERVICE_ENDPOINT]')
    parser.add_argument('-r', '--handle-resolver-url', type=str,
                        action='store', dest='handle_resolver_url',
    ...
    parser.add_argument('--resolve-url-template-archive', type=str,
                        action='store', dest='resolve_url_template_archive',
                        help="Template for the URL that a DIP's PURL should"
                        " resolve to", default=None)
    parser.add_argument('--resolve-url-template-mets', type=str, action='store',
                        dest='resolve_url_template_mets', help="Template for"
                        " the URL that a DIP's PURL with the \"mets\" qualifier"
                        " should resolve to.", default=None)
    parser.add_argument('--resolve-url-template-file', type=str, action='store',
                        dest='resolve_url_template_file', help="Template for"
                        " the URL that a file's PURL should resolve to (Note:"
                        " defaults to access)", default=None)
    parser.add_argument('--resolve-url-template-file-access', type=str,
                        action='store', dest='resolve_url_template_file_access',
                        help="Template for the URL that a file's PURL with the"
                        " \"access\" qualifier should resolve to", default=None)
    parser.add_argument('--resolve-url-template-file-preservation', type=str,
                        action='store',
                        dest='resolve_url_template_file_preservation',
                        help="Template for the URL that a file's PURL with the"
                        " \"preservation\" qualifier should resolve to",
                        default=None)
    parser.add_argument('--resolve-url-template-file-original', type=str,
                        action='store',
